import React, { useState } from 'react';
import { motion } from 'motion/react';

export const RockBanner: React.FC = () => {
  const [hoveredLetter, setHoveredLetter] = useState<string | null>(null);

  const words = [
    { word: 'KAMAL', color: '#a855f7' },
    { word: 'AZAM', color: '#c084fc' }
  ];

  return (
    <section className="relative py-14 sm:py-24 bg-[#08090b] overflow-hidden select-none border-y border-white/5">
      {/* Subtle background ambient grid */}
      <div className="absolute inset-0 opacity-[0.03] bg-[radial-gradient(#fff_1px,transparent_1px)] [background-size:24px_24px] pointer-events-none" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-8 md:gap-12">
          {words.map((w, wIdx) => (
            <div key={wIdx} className="flex items-center justify-center space-x-1 sm:space-x-3">
              {w.word.split('').map((char, cIdx) => {
                const letterKey = `${wIdx}-${cIdx}`;
                const isHovered = hoveredLetter === letterKey;
                return (
                  <motion.div
                    key={letterKey}
                    onMouseEnter={() => setHoveredLetter(letterKey)}
                    onMouseLeave={() => setHoveredLetter(null)}
                    whileHover={{ scale: 1.06, y: -6 }}
                    transition={{ type: 'spring', stiffness: 450, damping: 25 }}
                    className="cursor-pointer transition-all duration-300"
                  >
                    <span
                      className="font-display font-black uppercase text-5xl sm:text-7xl md:text-8xl lg:text-9xl tracking-tight block"
                      style={{
                        WebkitTextStroke: isHovered
                          ? `2px ${w.color}`
                          : '1.5px rgba(255, 255, 255, 0.75)',
                        color: isHovered ? w.color : 'transparent',
                        textShadow: isHovered ? `0 0 40px ${w.color}80` : 'none',
                        transition: 'all 0.3s cubic-bezier(0.16, 1, 0.3, 1)',
                      }}
                    >
                      {char}
                    </span>
                  </motion.div>
                );
              })}
            </div>
          ))}
        </div>

        {/* Ambient ticker subline matching kamalazam.com */}
        <div className="mt-8 flex flex-wrap items-center justify-center gap-3 sm:gap-6 text-xs sm:text-sm font-mono tracking-widest text-neutral-400 uppercase">
          <span>01 Video Production</span>
          <span className="text-[#a855f7]">•</span>
          <span>02 Photography</span>
          <span className="text-[#a855f7]">•</span>
          <span>03 Social Management</span>
          <span className="text-[#a855f7]">•</span>
          <span>04 Media Buying</span>
        </div>
      </div>
    </section>
  );
};
