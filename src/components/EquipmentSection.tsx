...
import { PRODUCTION_SERVICES_DATA } from '../data/rockMediaData';
...
export const EquipmentSection: React.FC<EquipmentSectionProps> = ({ onOpenBooking }) => {
  ...
  const filteredServices = PRODUCTION_SERVICES_DATA.filter((service) => {
            ...
            {categories.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center space-x-2 px-4 sm:px-5 py-2.5 rounded-full text-xs font-semibold uppercase tracking-wider transition-all duration-300 whitespace-nowrap cursor-pointer border ${
                    isActive
